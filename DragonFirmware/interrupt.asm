...
;		sta		SecVecSWI
		...
		jsr		MMC_GetRAMCTRL			; Go get current value
		anda	#~(D_NMI_ENABLE+D_RAM_VEC+D_RAM_VEC_WP) ; Disable NMI + Select rom vectors, remove write prot on RAM vectors
		pshs	a						; save current RAM_CTRL
		jsr		MMC_SetRAMCTRL			; and update
		
;       lda     D_RAM_CTRL              ; Get ram control       
;		anda	#~(D_NMI_ENABLE+D_RAM_VEC+D_RAM_VEC_WP) ; Disable NMI + Select rom vectors, remove write prot on RAM vectors
;		sta		D_RAM_CTRL
;		pshs	a						; save current RAM_CTRL
...
;
; 2024-11-07, Modified to do a byte at a time, yes a word at a time is fatser, but for
; 16 bytes worth this is not going to be significant.
; The reason for this change is that the Pico code uses a read of the reset vector bytes one 
; after the other to detect a reset (as this is how it is fetched at reset time). 
; Unfortunately moving the vectors a word at a time will also trigger this causing the Pico
; to think it is getting a reset, and clearing it's copy of the SAM registers.
; By doing this a byte at a time, it should not trigger this behavior as there will be 
; instruction fetches between the two vector bytes.
 
		lda		#HWVecCount*2			; No of vectors * bytes/vector
...
;        stx     HWVecSWI
		...
		puls	a						; recover saved  RAM_CTRL
        ora		#D_NMI_ENABLE+D_RAM_VEC+D_RAM_VEC_WP	; Enable NMI routing + RAM vectors, write protect RAM vectors.
		jsr		MMC_SetRAMCTRL			; and update
...
;
NewNMI
; Disable NMI so we don't get called recursively
		jsr		MMC_GetRAMCTRL			; Go get current value
...
; New code for Pico, read SAM bits from pico
		lda		#CMD_GET_SAMBITS
		jsr		MMC_SendCmd				; Send the command, and wait for it.
		
		lda		#CMD_INIT_READ			; read the data
		jsr		MMC_SendCmdRaw			
		
		lda		D_READ_DATA_REG			; get MSB
		ldb		D_READ_DATA_REG			; get LSB

...
;
; 2024-10-02	Made changes to port to running on the Raspbery Pi Pico 2, these included:
;					Merging the read and write data register into one.
;					Adding a one byte latch read register for single byte results which
;					saves having to send the read_data command to get the result.
;					Removed RAM control register and replaced it with commands to get and
;					set it.
;					Removed PIA replacement and tape emulation code as no longer required.
;
; 2024-11-07	All commands now return a result, in result register.
;					Busy flag changed to bit 7 so we can use a simple tst/bmi loop.
;
;	
...
DO_Select
		...
		bsr		MMC_GetRAMCTRL			; Get ram control reg

        anda    #D_SEL_DRAGON           ; Default to dragon
        
        pshs	a
		jsr     >MachineDetect          ; Detect the machine
		puls	a						; note doesn't change flags
        beq     InitDragon
        
        ora     #D_SEL_COCO             ; Select CoCo version
        
InitDragon
		bsr		MMC_SetRAMCTRL			; Update it
...
MMCLC054
		JSR		>SetPlatform			; Tell the Pico what our platform is
...
CmdHelpMain
		...
		jsr	    >MMC_SendCmdLatchRead	; send command get version			

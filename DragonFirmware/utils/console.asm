...
; Set CONDEB to 1 to include console debugging code usefull for debugging
; other routines, but not needed when building release version.
CONDEB	equ		1	
...
CON_HexByteAt0
		...
		pshs	x							; save it
		ldx		#$400						; pos 0
		stx     <TextVDUCursAddr            ; set cursor pos
		
		bsr		CON_WriteHexByte			; write it
		
		puls	x							; restore old cursor pos
		stx     <TextVDUCursAddr            ; set cursor pos
...
CON_NdotN
		...
		jsr		CON_GetHexNibbleA           ; send msn
		jsr		BasicScreenOut	
		...
		lda		,s				            ; retrive digit
		jsr		CON_GetHexNibbleA       	; send lsn
		jsr		BasicScreenOut	

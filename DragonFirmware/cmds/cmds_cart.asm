...
; the rom out. Though until the call is made to set the RAM_CTRL reg,
; we can still call ROM routines.

CmdCartRAMCode:
;		lda     D_RAM_CTRL              ; get current control register

		jsr		MMC_GetRAMCTRL          ; get current control register
...
CmdCartRAMNoAuto:
		jsr		MMC_WaitPutLatchRead	; Put the new D_RAM_CTRL in the latch
		
		lda		#CMD_SET_RAM_CTRL		; Set it.
		sta		D_CMD_REG				
CmdCartWait		
		lda		D_STATUS_REG			; Wait for command to exec
		bmi		CmdCartWait

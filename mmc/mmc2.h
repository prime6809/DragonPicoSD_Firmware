...
extern const char __in_flash()  CompileTime[];
...
//#define ResetLatch(latch)			do {(latch.Index = 0); printf("LatchReset at :%s line:%d\n",__FILE__,__LINE__); } while(0)
#define ResetLatch(latch)			  (latch.Index = 0)
//#define ResetLatch(latch)
#if 1
...
extern volatile unsigned char globalPlatform;
...
extern void DoCommand(uint8_t Cmd);

#define MODEREAD 1
#define MODEWRITE 2


typedef uint8_t (*WORKERFN)(void);

#if 0
#define WFUNC(x)  extern void wfn##x(void); const WORKERFN WFN_##x = wfn##x;
#else
#define WFUNC(x)  extern uint8_t wfn##x(void); const WORKERFN WFN_##x = wfn##x;
#endif
